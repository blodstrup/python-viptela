...
def fetch_aggregation_raw(session, host, device_ips, hours=1):
    """
    POST to the App-Route FEC aggregation endpoint and return the full JSON response.
    Uses the correct 'local_system_ip' field for querying.
    """
...
def main():
    ...
    # 4) Pretty-print the full JSON structure
    print("=== Full JSON Response ===\n")
    print(json.dumps(raw, indent=2))

    # 5) List all unique fields in each record under "data"
    data = raw.get("data", [])
    fields = set()
    for rec in data:
        fields.update(rec.keys())

    print("\n=== Fields returned in each 'data' record ===")

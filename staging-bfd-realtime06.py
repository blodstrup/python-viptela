...
def main():
    ...
    hdr = f"{'local-systemip':<18} {'local-color':<20} {'remote-systemip':<18} {'remote-color':<20} {'LOSS%':>8} {'LAT(ms)':>10} {'JIT(ms)':>10}"
    print(hdr)
    print("-" * len(hdr))
    ...
    for rec in sla_data:
        ...
        print(f"{lip:<18} {lcol:<20} {rip:<18} {rcol:<20} "
              f"{loss:8.2f} {latency:10.2f} {jitter:10.2f}")

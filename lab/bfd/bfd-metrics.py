...
def fetch_sla_for_device(session, host, device_ip, hours=1):
    """
    Query the App-Aware Routing Aggregation API for a single device,
...
def main():
    ...
    # 3) Fetch all devices, filter out control-plane personalities
    ...
    for d in all_devs:
        ...
        personality = d.get("personality", "").lower()
        # require a real IP, skip control-plane roles
        if ip and ip != "0.0.0.0" and personality not in ("vmanage", "vsmart", "vbond"):
            edges.append(ip)
